...
impl ApplicationManager {
    ...
    fn render_header(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ui.label(
                egui::RichText::new("APPLICATION MIXER")
                    .size(16.0)
    ...
    fn render_application_list(&mut self, ui: &mut egui::Ui) {
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let app_indices: Vec<usize> = self
                    .applications
                    .iter()
                    .enumerate()
                    .filter_map(|(i, app)| {
                ...
                if self.auto_detect {
                    ...
                    let detected_apps: Vec<String> = self
                        .detected_apps
                        .iter()
                        .filter_map(|(name, app)| {
                            if app.is_running {
        ...
        if let Ok(output) = Command::new("ps").args(["-eo", "pid,comm"]).output() {
            ...
            for app in &mut self.applications {
                ...
                for line in ps_output.lines() {
                    if line
                        .to_lowercase()
                        .contains(&app.process_name.to_lowercase())
                    {
                        app.is_running = true;
                        if let Ok(pid) =
                            line.split_whitespace().next().unwrap_or("0").parse::<u32>()
                        {
                            app.pid = Some(pid);
                        }
                        break;
            ...
            if ps_output.to_lowercase().contains(process) {
                ...
                if !self.detected_apps.contains_key(&app_name)
                    && !self
                        .applications
                        .iter()
                        .any(|a| a.process_name.to_lowercase() == *process)
                {
