...
impl MixerPanel {
    ...
    fn render_mixer_tabs(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            if ui
                .add(widgets::glow_button("MONITOR", monitor_color))
                .clicked()
            {
            ...
            if ui
                .add(widgets::glow_button("STREAM", stream_color))
                .clicked()
            {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let matrix_text = if self.show_routing_matrix {
                    "SIMPLE"
                } else {
                    "MATRIX"
                };
                if ui
                    .add(widgets::glow_button(
                        matrix_text,
    ...
    fn render_output_selector(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| {
            ...
            ui.label("Master:");
            ui.add(
                egui::Slider::new(&mut self.outputs[self.selected_output].volume, 0.0..=1.0)
                    .step_by(0.01)
                    .show_value(false),
            );
        });
            ...
            for (i, channel_name) in channel_names.iter().enumerate() {
                ui.vertical(|ui| {
                    ...
                    if ui
                        ...
                        .changed()
                    {
                        self.outputs[self.selected_output]
                            .channels
                            .insert(i, volume);
                    }
            ...
            .show(ui, |ui| {
                ...
                for output in self.outputs.iter_mut() {
                    ...
                    for channel_idx in 0..channel_names.len() {
                        let current_level =
                            output.channels.get(&channel_idx).copied().unwrap_or(0.0);
